...
*╰──────────●●►*`;
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        try {
            ...
            const botName = userConfig.botName || 'JANI-MD';

            // =================================================================
            //  PART A: MASSIVE STATIC DATABASE (Manually Added Words)
            // =================================================================
            let replies = [];
            const add = (words, reply, react) => {
                words.forEach(word => replies.push({ word, reply, react }));
            };

            // --- 1. GREETINGS (ENGLISH / SINGLISH / SINHALA) ---
            add(['hi', 'hii', 'hiii', 'hie', 'hy', 'hye', 'hay', 'hei', 'heyy', 'yo', 'yoo', 'haai'], 'හායි! කොහොමද ඉතින්? 😃', '👋');
            add(['hello', 'helo', 'hlo', 'allo', 'alo', 'hallow', 'holla', 'halo'], `Hello! ${botName} වෙත සාදරයෙන් පිළිගනිමු! 🙏`, '🙏');
            add(['ayubowan', 'ayubown', 'aayubowan', 'aaayubowan', 'ayubowa', 'ආයුබෝවන්'], 'ආයුබෝවන් යාලුවා! තෙරුවන් සරණයි! 🌸', '🙏');
            add(['vanakkam', 'vannakkum', 'vanakam'], 'Vanakkam! Welcome! 🤝', '🤝');
            add(['suba dawasak', 'suba dwsak', 'suba davask', 'good day', 'සුබ දවසක්'], 'ඔයාටත් සුබම සුබ දවසක් වේවා! 🌈', '🌥️');

            // --- 2. STATUS & CHAT START ---
            add(['kohomada', 'kohomda', 'khomada', 'khomda', 'komada', 'komda', 'kohomdo', 'koheda', 'කොහොමද'], 'මම හොඳින්. ඔයාට කොහොමද? 🙃', '🥰');
            add(['sapa', 'sapada', 'sapa saneepa', 'sepada', 'sepa', 'sapada kiyanna', 'sapa kan', 'සැපෙන්ද'], 'සැප සනීප හොඳයි! ඔයාට කොහොමද? 😎', '😎');
            add(['moko', 'mk', 'mokada', 'mokda', 'mkda', 'mokada karanne', 'mk krnne', 'moko wenne', 'මොකෝ'], 'නිකන් ඉන්නවා අනේ.. ඔයා? 🤷‍♂️', '👀');
            add(['innawa', 'innwa', 'inwa', 'mn innawa', 'api innawa', 'inna', 'hmm innawa', 'ඉන්නවා'], 'එල එල.. සතුටින් ඉන්න ඕන! 😊', '👍');
            add(['shape', 'shape eke', 'shp', 'shp eke', 'shape eke innawa', 'ශේප්'], 'Shape නම් එල. 😎', '👌');
            add(['aulak na', 'aulak ne', 'awulak na', 'awulak ne', 'kisima aulak na', 'අවුලක් නෑ'], 'එල එල අවුලක් නැත්තම් හොඳයි. 👍', '👍');

            // --- 3. FOOD & DRINK ---
            add(['kawada', 'kaawada', 'kewada', 'kewa', 'kawe', 'kewad', 'kewda', 'kewada oya', 'කෑවද'], 'ඔව් මම කෑවා. ඔයා කෑවද? 🍛', '🍛');
            add(['biwada', 'biwwada', 'bwwada', 'biuwa', 'wathura biwada', 'biuva', 'බිව්වද'], 'වතුර නම් බිව්වා. 💧', '🥤');
            add(['tea', 'the', 'te', 'tea biwada', 'the biwada', 'tea biwwada', 'තේ බිව්වද'], 'තේ බිව්වද? ☕', '☕');
            add(['badagini', 'badaginiyi', 'badaginiy', 'badaginie', 'bada gini', 'badaginiyo', 'බඩගිනියි'], 'අයියෝ.. ගිහින් මොනවා හරි කන්නකෝ. 🍔', '🍔');
            add(['monada kawe', 'mk kawe', 'mokada kewe', 'monada kewe', 'kama', 'monada kema'], 'මම කන්නේ Data සහ Current! ⚡', '⚡');
            add(['kanna', 'kamu', 'yamu kanna', 'kanna yamu', 'bath kamu', 'කමු'], 'හා යමු යමු! 🍕', '🍕');

            // --- 4. TIME & SLEEP ---
            add(['gm', 'g m', 'good morning', 'gd mrng', 'suba udasanak', 'gud mrng', 'morning'], 'Good Morning! සුබ උදෑසනක් වේවා! ☀️', '☀️');
            add(['gn', 'g n', 'good night', 'gd n8', 'suba rathriyak', 'gud nyt', 'gn sd', 'nyt'], 'Good Night! සුබ රාත්‍රියක් වේවා! 😴', '🌙');
            add(['nidi', 'nidi nadda', 'nidimathai', 'nidimatha', 'nidi mathe', 'nidaganna', 'නිදිමතයි'], 'නින්ද යන්නේ නැද්ද? ගිහින් නිදාගන්න. 😴', '😴');
            add(['bs', 'b s', 'budu saranai', 'budusaranai', 'theruwan saranai', 'budu saranay', 'බුදු සරණයි'], 'බුදු සරණයි යාලුවා! 🙏', '🙏');
            add(['jesu pihitai', 'jesu pihitay', 'devipihitai', 'devi pihitai', 'ජේසු පිහිටයි'], 'ජේසු පිහිටයි! ✝️', '✝️');
            add(['tc', 'take care', 't c', 'parissamin', 'parissamen', 'parissamin inna', 'පරිස්සමින්'], 'Take Care! පරිස්සමින් ඉන්න. ❤️', '❤️');

            // --- 5. LOVE & RELATIONSHIPS ---
            add(['adarei', 'aadarei', 'adary', 'love', 'luv', 'lub', 'adare', 'aadare', 'ආදරෙයි'], 'මාත් ඔයාට ගොඩක් ආදරෙයි! ❤️ 🙈', '❤️');
            add(['manika', 'manik', 'menika', 'mage manika', 'sudda', 'suddi', 'mage pana', 'මැණික'], 'ඇයි මගේ මැණික? 💎', '💎');
            add(['sudu', 'suduu', 'suduh', 'mage sudu', 'සුදු'], 'ඇයි සුදු? 😍', '😍');
            add(['baba', 'babah', 'bab', 'mage baba', 'baboo', 'bbaa', 'බබා'], 'ඔව් කියන්න බබා... 😘', '😘');
            add(['miss you', 'miss u', 'ms u', 'miss yu', 'miss you so much', 'miss'], 'I miss you too! 😢', '🥺');
            add(['umma', 'ummah', 'ummm', 'ummmm', 'umah', 'ummmma', 'උම්මා'], 'Ummmma! 😘', '💋');
            add(['crush', 'crush eka', 'mage crush', 'crush innawada', 'crush kawda'], 'මට නම් Crush එකක් නෑ.. මම Bot කෙනෙක්නේ. 🤖', '🤖');
            add(['kella', 'kelle', 'kello', 'kell', 'kolla', 'kollo', 'kolle', 'කෙල්ල'], 'ආ.. මතක් උනාද? 😉', '😉');
            add(['badimu', 'marriage', 'kasada', 'bandimu', 'kasada badimu', 'බඳිමු'], 'මම තාම පොඩියි අනේ.. 🙈', '🙈');
            add(['boot', 'boot kala', 'boot thibba', 'boot ekak', 'boot damma', 'බූට්'], 'අයියෝ.. ගණන් ගන්න එපා. හොඳ කෙනෙක් ලැබෙයි. 💔', '💔');

            // --- 6. SLANG / FUNNY / AGGRESSIVE ---
            add(['pissu', 'pissuda', 'pisu', 'pisuda', 'pssu', 'pssuda', 'pissu hadenawa', 'පිස්සු'], 'ඔව් ඔව් ඔයාට වගේම මටත් පිස්සු! 🤪', '🤪');
            add(['gon', 'gona', 'gone', 'gonn', 'gon haraka', 'gon part', 'ගොන්'], 'තණකොළ ඕනද? 🌿', '🌿');
            add(['buru', 'buruwa', 'booru', 'booruwa', 'booruwo', 'me buruwa', 'බූරුවා'], 'තමුසෙ බූරුවා! 🐴', '🐴');
            add(['hutto', 'hutta', 'huththa', 'huththo', 'hththa', 'hththo', 'hutth', 'හුත්තෝ'], 'කුණුහරප කියන්න එපා මහත්තයෝ.. කැතයි ඈ! 🤬', '🤐');
            add(['pakaya', 'paka', 'pako', 'pkaya', 'pka', 'pake', 'pke', 'පකයා'], 'කට කට.. මොනවද මේ කියන්නේ? 😤', '🤫');
            add(['kari', 'kariya', 'kriya', 'kri', 'kary', 'kariyo', 'කැරියා'], 'ඔහොම කතා කරන්න එපා යාලුවා. 😔', '😔');
            add(['ponnaya', 'ponnya', 'ponayo', 'ponaya', 'ponna', 'පොන්නයා'], 'කණ්ණාඩියක් බලපන්. 😂', '🖕');
            add(['ado', 'adoo', 'adoh', 'yako', 'yakoo', 'yakoh', 'යකෝ'], 'මොකද යකෝ කෑගහන්නේ? 🙉', '🙉');
            add(['bijja', 'bijjo', 'bijj', 'topa', 'tope', 'බිජ්ජ'], 'කට පරිස්සම් කරගන්න! 🤫', '🤫');
            add(['wesa', 'wesiye', 'wesi', 'balli', 'balla', 'වේසි'], 'ඔහොම කතා කරන එක කැතයි නේද? 😒', '😒');
            add(['modaya', 'moda', 'modayo', 'moda yaka', 'මෝඩයා'], 'මම මෝඩ නෑ.. ඔයා තමයි මෝඩයා! 😜', '😜');

            // --- 7. BOT INFO & TECH ---
            add(['bot', 'bott', 'bota', 'robot', 'queen seya', 'queen seya xmd'], `ඔව් මම ${botName}. උදව්වක් ඕනද? 🤖`, '🤖');
            add(['nama', 'name', 'oyage nama', 'name mokakda', 'nama mokakda'], `මගේ නම ${botName}. 👸`, '👸');
            add(['owner', 'ownr', 'kawda owner', 'ayithi', 'kawda haduwe', 'owner kawda'], 'මගේ අයිතිකරු ගැන දැනගන්න .owner ගහන්න. 👨‍💻', '👨‍💻');
            add(['menu', 'list', 'commands', 'cmd', 'help'], 'Type *.menu* to see my commands! 📜', '📜');
            add(['alive', 'online', 'system', 'status'], 'Yes, I am alive and running! 🟢', '🟢');
            add(['group', 'grp', 'group link', 'support'], 'Type *.group* or *.support* for group link. 👥', '👥');
            add(['restart', 'reboot', 'reset'], 'Type *.restart* to restart me. 🔄', '🔄');

            // --- 8. COMMON SHORT WORDS ---
            add(['ow', 'owu', 'ov', 'owww', 'owhm', 'hmm ow', 'ow ow', 'ඔව්'], 'එකෙන්ම! 😎', '✅');
            add(['na', 'ne', 'naha', 'neh', 'naa', 'neha', 'nhe', 'ne ne', 'නෑ'], 'ඒ මොකද ඒ? 🤔', '❌');
            add(['hari', 'hri', 'harii', 'hry', 'hari hari', 'hari ehenam', 'හරි'], 'එල කිරි! 🥛', '🥛');
            add(['ela', 'elaa', 'elaz', 'elzz', 'elama', 'elakiri', 'elz', 'එල'], 'ගැම්මක් තමයි! 💪', '🔥');
            add(['hmm', 'hmmm', 'hm', 'hmmmm', 'hmmmmm', 'hm hm'], 'හ්ම්ම්... කියන්නකෝ ඉතින්. 🥱', '🥱');
            add(['aha', 'ahh', 'ah', 'aah', 'ahhh', 'ahh ehemada'], 'ආහ්.. මොකද වෙන්නේ? 😉', '😉');
            add(['epa', 'epaa', 'epo', 'epaaa', 'එපා'], 'එපා නම් එපා.. මම යනවා! 🚶‍♂️', '😒');
            add(['ba', 'baa', 'baha', 'be', 'beh', 'ba ba', 'බෑ'], 'ඇයි බැරි? පුළුවන්! 💪', '💪');
            add(['ane', 'anee', 'aney', 'aneee', 'ane manda', 'අනේ'], 'අනේ මන්ද ඔයත්.. 🤭', '🤭');
            add(['ai', 'ayi', 'ay', 'aiy', 'why', 'yi', 'ai e', 'ඇයි'], 'ඇයි අහන්නේ? 🤔', '🤔');
            add(['ok', 'okk', 'okay', 'oky', 'k', 'kk', 'ok ok'], 'Ok boss! 👌', '👌');
            add(['bye', 'byee', 'by', 'tata', 'bye bye'], 'Bye Bye! See you later! 👋', '👋');
            add(['thanks', 'tnx', 'tks', 'thank you', 'thnx', 'thank u'], 'You are welcome! 🤗', '🤗');
            add(['sorry', 'sry', 'sory', 'sorru', 'sry thama'], 'සමාවෙන්න ඕන නෑ යාලුවා. 😇', '😇');

            // --- 9. EMOTIONS & ENTERTAINMENT ---
            add(['lol', 'lmao', 'haha', 'hahaha', 'he he', 'hik', 'hik hik'], 'Hahaha! 😂', '😂');
            add(['sad', 'duk', 'dukai', 'duka', 'upset', 'awulen', 'දුකයි'], 'දුක් වෙන්න එපා.. සතුටින් ඉන්න! 😊', '😊');
            add(['happy', 'sathutu', 'sathutui', 'sathutin', 'සතුටුයි'], 'සතුටින් ඉන්න එක තමයි වටින්නේ! 😁', '😁');
            add(['angry', 'taraha', 'tharaha', 'tharahai', 'තරහ'], 'තරහ ගන්න එපා අනේ.. Cool වෙන්න! 🧊', '🧊');
            add(['boring', 'kammali', 'kammalie', 'kammali nadda', 'කම්මැලියි'], 'Film එකක් බලමුද? 🎬', '🎬');
            add(['film', 'movie', 'tv series', 'movies'], 'මොන Film එකද බලන්නේ? 🎥', '🎥');
            add(['song', 'sindu', 'music', 'songs', 'සින්දු'], 'සින්දුවක් අහමුද? 🎵', '🎵');
            add(['game', 'gaming', 'ff', 'pubg', 'free fire'], 'Game ගහනවද? එල එල! 🎮', '🎮');
            add(['fun', 'athal', 'fun ekak', 'joke'], 'ජෝක් එකක් කියන්නද? 😂', '😂');

            // --- 10. SCHOOL & WORK ---
            add(['study', 'padam', 'exam', 'class', 'iskole', 'පාඩම්'], 'හොඳට පාඩම් කරන්න! Good Luck! 📚', '📚');
            add(['work', 'wada', 'job', 'office', 'wada ban', 'වැඩ'], 'මහන්සි වෙලා වැඩ කරන්න! 💪', '💪');
            add(['school', 'uni', 'campus', 'පාසල්'], 'ඉගෙන ගන්න එක වටිනවා! 🎓', '🎓');

            // --- 11. ASTROLOGY & LIFE ---
            add(['kendra', 'lagna', 'rahu', 'kethu', 'නැකත්'], 'කේන්දර විශ්වාස කරනවද? 🔮', '🔮');
            add(['jeewithe', 'life', 'adare', 'ජීවිතේ'], 'ජීවිතේ සුන්දරයි! 🌸', '🌸');
            add(['salli', 'money', 'cash', 'සල්ලි'], 'සල්ලි තමයි ප්‍රශ්නෙ! 💸', '💸');

            // =================================================================
            //  LOGIC: CHECK STATIC DB -> IF NOT FOUND -> ASK AI
            // =================================================================
            let finalReply = '';
            let finalReact = '';

            // 1. Check Static Database First (Fastest)
            const match = replies.find(r => 
                msgText === r.word || 
                msgText.startsWith(`${r.word} `) || 
                msgText.endsWith(` ${r.word}`) || 
                msgText.includes(` ${r.word} `)
            );

            if (match) {
                finalReply = match.reply;
                finalReact = match.react;
            } else {
                // 2. Fallback to AI (If word not in list)
                try {
                    // Hercai GPT API (Free)
                    const aiUrl = `https://hercai.zaidrake.xyz/v2/hercai?question=${encodeURIComponent(msgText)}`;
                    const response = await axios.get(aiUrl);
                    
                    if (response.data && response.data.reply) {
                        finalReply = response.data.reply;
                        finalReact = '🤖'; // Robot React for AI
                    }
                } catch (aiError) {
                    // If AI fails or no internet, do nothing (Silent)
                    return; 
                }
            }

            // =========================================================
            //  SEND REPLY (FAKE META HEADER - NO PHOTO)
            // =========================================================
            if (finalReply) {
                if (finalReact) {
                    await socket.sendMessage(from, { react: { text: finalReact, key: msg.key } });
                }

                // Create the Fake Meta AI Header
                const fakeMetaQuote = {
                    key: {
                        fromMe: false,
                        participant: "0@s.whatsapp.net",
                    ...
                    message: {
                        extendedTextMessage: {
                            text: 'JANI MD", // Custom Header Name
                            title: "Meta AI",
                        }
                    }
                };
